...
passport.use(
    new BasicStrategy( async (email, password, done)=>{
        try{
            ...
            if(!await bcrypt.compare(password, user.contraseña)){ //Bcrypt compares the sended password with the user's stored password
                return done(boom.unauthorized(), false); //If They dont match, Unauthorized
            }

            delete user.contraseña; //Password field is deleted for avoiding security issues
